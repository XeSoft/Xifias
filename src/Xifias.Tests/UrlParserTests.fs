...
type UrlParsing() =
    ...
    let parse = Xifias.UrlParser.parse



    [<TestMethod>]
    member __.``parse ignores leading and trailing slashes`` () =
        let actual = parse (s "test") id q "test" |> Option.map (fun f -> f 1)
        ...
        let actual = parse (s "test") id q "/test/" |> Option.map (fun f -> f 1)
        areEqual (Some 1) actual
        let actual = parse (s "test") id q "/test/foo" |> Option.map (fun f -> f 1)
    ...
    *)
    ...
    member __.``s parser returns None when provided string does not match`` () =
        let actual = parse (s "test") id q "/grade" |> Option.map (fun f -> f 1)
        areEqual None actual
